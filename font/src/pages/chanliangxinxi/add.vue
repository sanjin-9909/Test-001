...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="收获时间" prop="shouhuoshijian">
				<el-date-picker
			...
			<el-form-item class="add-item" label="收获数量(吨/㎡)" prop="shouhuoshuliang">
				<el-input v-model.number="ruleForm.shouhuoshuliang" 
					placeholder="收获数量(吨/㎡)" clearable :readonly="ro.shouhuoshuliang"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`chanliangxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`chanliangxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
