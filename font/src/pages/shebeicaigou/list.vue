			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('shebeicaigou','新增')" type="primary" @click="add('/index/shebeicaigouAdd')">
  					...
  					<el-button class="list-static-btn" v-if="btnAuth('shebeicaigou','设备采购费用')" type="success" @click="chartDialogShow1">
				...
				<div class="list3 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box" >
						...
						<div class="item-info">
							...
							<div class="desc ql-snow ql-editor" v-html="item.caigoubeizhu"></div>
						</div>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("shebeicaigou/value/shebeimingcheng/zongjine",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
