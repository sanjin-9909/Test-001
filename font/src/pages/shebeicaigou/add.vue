...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="采购价格" prop="caigoujiage">
				<el-input-number v-model="ruleForm.caigoujiage" placeholder="采购价格" :disabled="ro.caigoujiage"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="采购数量" prop="shuliang">
				<el-input v-model.number="ruleForm.shuliang" 
					placeholder="采购数量" clearable :readonly="ro.shuliang"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`shebeicaigou/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('shebeicaigou/list', {
									params: params
								}).then(res => {
