	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/hezuoshe?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.chengliriqi}}</div>
					</div>
					<div class="item">
						<div class="lable">联系人</div>
						<div class="text "  >{{detail.lianxiren}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">主营项目</div>
						<div class="text "  >{{detail.zhuyingxiangmu}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('hezuoshe','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('hezuoshe','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('hezuoshe','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="合作社详情" name="1">
					<div class="ql-snow ql-editor" v-html="detail.hezuoshexiangqing"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.hezuoshemingcheng;
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/hezuoshe', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/hezuosheAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此合作社？') .then(_ => {
					this.$http.post('hezuoshe/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
