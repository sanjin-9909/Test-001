...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="维修进度" prop="weixiujindu">
				<el-select v-model="ruleForm.weixiujindu" placeholder="请选择维修进度" :disabled="ro.weixiujindu"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="报修详情" prop="baoxiuxiangqing">
				<editor 
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					this.ruleForm.weixiujindu = '未维修'; 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`guzhangbaoxiu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('guzhangbaoxiu/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`guzhangbaoxiu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
