	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="作物类型" prop="zuowuleixing">
				<el-input v-model="ruleForm.zuowuleixing" 
					placeholder="作物类型" clearable :readonly="ro.zuowuleixing"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="灌溉数量" prop="guangaishuliang">
				<el-input v-model="ruleForm.guangaishuliang" 
					placeholder="灌溉数量" clearable :readonly="ro.guangaishuliang"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="灌溉时间" prop="guangaishijian">
				<el-input v-model="ruleForm.guangaishijian" 
					placeholder="灌溉时间" clearable :readonly="ro.guangaishijian"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="灌溉周期" prop="guangaizhouqi">
				<el-input v-model="ruleForm.guangaizhouqi" 
					placeholder="灌溉周期" clearable :readonly="ro.guangaizhouqi"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="灌溉方案" prop="guangaifangan">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="灌溉方案"
				...
				<el-button type="primary" size="small" @click="AILongClick('guangaifangan', '灌溉方案', '32')">AI生成</el-button>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`guangaijilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`guangaijilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
