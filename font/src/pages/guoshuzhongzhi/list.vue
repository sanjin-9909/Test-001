<template>
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-select v-model="formSearch.zuowuleixing" placeholder="请选择作物类型" :clearable="true">
						<el-option v-for="(item, index) in zuowuleixingOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('guoshuzhongzhi','新增')" type="primary" @click="add('/index/guoshuzhongzhiAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('guoshuzhongzhi','监测数据')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						监测数据
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('guoshuzhongzhi','热门作物')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						热门作物
					</el-button>
			...
			<div class="list">
				...
				<div class="list3 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box" >
						<div class="img">
							<img @click.stop="imgPreView(item.zuowutupian)" v-if="item.zuowutupian && item.zuowutupian.substr(0,4)=='http'&&item.zuowutupian.split(',w').length>1" :src="item.zuowutupian" class="image" />
							<img @click.stop="imgPreView(item.zuowutupian.split(',')[0])" v-else-if="item.zuowutupian && item.zuowutupian.substr(0,4)=='http'" :src="item.zuowutupian.split(',')[0]" class="image" />
							<img @click.stop="imgPreView(baseUrl + (item.zuowutupian?item.zuowutupian.split(',')[0]:''))" v-else :src="baseUrl + (item.zuowutupian?item.zuowutupian.split(',')[0]:'')" class="image" />
						</div>
						<div class="item-info">
							<div>
								<div class="name">{{item.zuowumingcheng}}</div>
								<div class="name">{{item.zuowuleixing}}</div>
								<div class="time_item">
							...
							<div class="desc ql-snow ql-editor" v-html="item.zhongzhixiangqing"></div>
						</div>
					</div>
				</div>
			</div>
		...
		<el-dialog
			...
			width="800">
			<div id="zuowuleixingChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	export default {
		...
		async created() {
			...
			await this.$http.get('option/zuowuleixing/zuowuleixing').then(res => {
				if (res.data.code == 0) {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.zuowuleixing != '') searchWhere.zuowuleixing = this.formSearch.zuowuleixing;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`guoshuzhongzhi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/guoshuzhongzhiDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("guoshuzhongzhi/valueMul/dapengmingcheng?yColumnNameMul=shiyiwendu,shiyishidu,guangzhao",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis1 = [];
							let yAxis1 = [];
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("guoshuzhongzhi/group/zuowuleixing",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chapterClick (row){
				...
				this.$router.push({path: '/index/chapterguoshuzhongzhi', query: params});
			},
		}
	}
</script>
