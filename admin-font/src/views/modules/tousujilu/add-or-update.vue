...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="投诉备注" prop="tousubeizhu" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.tousubeizhu" 
					class="editor"
					myQuillEditor="tousubeizhu"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.tousubeizhu" label="投诉备注" prop="tousubeizhu"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.tousubeizhu"></span>
			</el-form-item>
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='tousubeizhu'){
							this.ruleForm.tousubeizhu = obj[o];
							this.ro.tousubeizhu = true;
							continue;
						}
						if(o=='nonghuzhanghao'){
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.tousubeizhu = this.ruleForm.tousubeizhu.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
