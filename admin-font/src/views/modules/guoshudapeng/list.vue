...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					...
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select" label="大棚类型" prop="dapengleixing">
						...
						<el-select clearable v-model="searchForm.dapengleixing" placeholder="请选择大棚类型" >
							<el-option v-for="(item,index) in dapengleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('guoshudapeng','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('guoshudapeng','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('guoshudapeng','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('guoshudapeng','申请')" @click="dapengshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已申请')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								申请
							</el-button>
							<el-button class="edit" v-if=" isAuth('guoshudapeng','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('guoshudapeng','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<dapengshenqing-cross-add-or-update v-if="dapengshenqingCrossAddOrUpdateFlag" :parent="this" ref="dapengshenqingCrossaddOrUpdate"></dapengshenqing-cross-add-or-update>



		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('guoshudapeng','大棚数量')">
...
<script>
	...
	import dapengshenqingCrossAddOrUpdate from "../dapengshenqing/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				dapengshenqingCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			dapengshenqingCrossAddOrUpdate,
		},
		methods: {
			statChartClick() {
				if(this.isAuth('guoshudapeng','大棚数量')) {
			...
			dapengshenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.dapengshenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshudapeng');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.dapengshenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.dapengshenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "guoshudapeng/group/dapengmingcheng",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].dapengmingcheng);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].dapengmingcheng
								})
							}
							var option = {};
