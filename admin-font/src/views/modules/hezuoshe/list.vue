			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"bold","height":"40px"}' class="item-label">合作社名称</label>
						<el-input v-model="searchForm.hezuoshemingcheng" placeholder="合作社名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('hezuoshe','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('hezuoshe','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-table-column :resizable='true' :sortable='true' prop="status" label="状态">
						<template slot-scope="scope">
							<el-switch
								...
								active-color="#10C17C"
								inactive-color="#ccc"
								@change="(e)=>hezuoshestatusChange(e,scope.row)">
							</el-switch>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('hezuoshe','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('hezuoshe','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('hezuoshe','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.hezuoshemingcheng!='' && this.searchForm.hezuoshemingcheng!=undefined){
					params['hezuoshemingcheng'] = '%' + this.searchForm.hezuoshemingcheng + '%'
				}
