...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="检修详情" prop="jianxiuxiangqing" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.jianxiuxiangqing" 
					class="editor"
					myQuillEditor="jianxiuxiangqing"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.jianxiuxiangqing" label="检修详情" prop="jianxiuxiangqing"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.jianxiuxiangqing"></span>
			</el-form-item>
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='jianxiuxiangqing'){
							this.ruleForm.jianxiuxiangqing = obj[o];
							this.ro.jianxiuxiangqing = true;
							continue;
						}
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jianxiuxiangqing = this.ruleForm.jianxiuxiangqing.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `tixingjianxiu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.tixingjianxiuCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
