	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="收获时间" prop="shouhuoshijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.shouhuoshijian" label="收获时间" prop="shouhuoshijian" >
					<el-input v-model="ruleForm.shouhuoshijian" placeholder="收获时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="收获数量(吨/㎡)" prop="shouhuoshuliang" >
					<el-input v-model.number="ruleForm.shouhuoshuliang" placeholder="收获数量(吨/㎡)" clearable  :readonly="ro.shouhuoshuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="收获数量(吨/㎡)" prop="shouhuoshuliang" >
					<el-input v-model="ruleForm.shouhuoshuliang" placeholder="收获数量(吨/㎡)" readonly></el-input>
				</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.shouhuoshijian = this.getCurDate()
		},
		methods: {
			imgPreView(url){
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shouhuoshuliang'){
							this.ruleForm.shouhuoshuliang = obj[o];
							this.ro.shouhuoshuliang = true;
							continue;
						}
						if(o=='nonghuzhanghao'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `chanliangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chanliangxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
