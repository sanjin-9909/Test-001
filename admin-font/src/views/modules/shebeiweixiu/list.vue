...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('shebeiweixiu','更换')" @click="genghuanjiluCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已更换')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								更换
							</el-button>
							<el-button class="btn8" v-if="isAuth('shebeiweixiu','投诉')" @click="tousujiluCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已投诉')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								投诉
							</el-button>
		...
		<genghuanjilu-cross-add-or-update v-if="genghuanjiluCrossAddOrUpdateFlag" :parent="this" ref="genghuanjiluCrossaddOrUpdate"></genghuanjilu-cross-add-or-update>
...
<script>
	...
	import genghuanjiluCrossAddOrUpdate from "../genghuanjilu/add-or-update";
	...
	export default {
		data() {
			return {
				...
				genghuanjiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			genghuanjiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.genghuanjiluCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.genghuanjiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.genghuanjiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			tousujiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
