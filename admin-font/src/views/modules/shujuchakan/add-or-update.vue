...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="异常原因" prop="yichangyuanyin" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.yichangyuanyin" 
					class="editor"
					myQuillEditor="yichangyuanyin"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yichangyuanyin" label="异常原因" prop="yichangyuanyin"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.yichangyuanyin"></span>
			</el-form-item>
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yichangyuanyin = this.ruleForm.yichangyuanyin.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shujuchakan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shujuchakanCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
