	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="更换时间" prop="genghuanshijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.genghuanshijian" label="更换时间" prop="genghuanshijian" >
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="更换备注" prop="genghuanbeizhu" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.genghuanbeizhu" 
					class="editor"
					myQuillEditor="genghuanbeizhu"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.genghuanbeizhu" label="更换备注" prop="genghuanbeizhu"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.genghuanbeizhu"></span>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.genghuanshijian = this.getCurDate()
		},
		methods: {
			imgPreView(url){
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='genghuanbeizhu'){
							this.ruleForm.genghuanbeizhu = obj[o];
							this.ro.genghuanbeizhu = true;
							continue;
						}
						if(o=='nonghuzhanghao'){
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.genghuanbeizhu = this.ruleForm.genghuanbeizhu.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `genghuanjilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.genghuanjiluCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
