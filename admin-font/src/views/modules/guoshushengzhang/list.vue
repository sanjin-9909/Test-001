...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('guoshushengzhang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('guoshushengzhang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('guoshushengzhang','查看')"
					:data="dataList"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('guoshushengzhang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('guoshushengzhang','产量信息')" @click="chanliangxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								产量信息
							</el-button>
							<el-button class="edit" v-if=" isAuth('guoshushengzhang','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('guoshushengzhang','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<chanliangxinxi-cross-add-or-update v-if="chanliangxinxiCrossAddOrUpdateFlag" :parent="this" ref="chanliangxinxiCrossaddOrUpdate"></chanliangxinxi-cross-add-or-update>



		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('guoshushengzhang','果蔬生长数据')">
					<div id="shengzhangjieduanChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	import chanliangxinxiCrossAddOrUpdate from "../chanliangxinxi/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				chanliangxinxiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			chanliangxinxiCrossAddOrUpdate,
		},
		methods: {
			statChartClick() {
				if(this.isAuth('guoshushengzhang','果蔬生长数据')) {
			...
			chanliangxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.chanliangxinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshushengzhang');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chanliangxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chanliangxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var shengzhangjieduanChart1 = echarts.init(document.getElementById("shengzhangjieduanChart1"),'macarons');
					let params = {}
					this.$http({
						url: "guoshushengzhang/group/shengzhangjieduan",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].shengzhangjieduan);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].shengzhangjieduan
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '果蔬生长数据'
							...
							// 使用刚指定的配置项和数据显示图表。
							shengzhangjieduanChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shengzhangjieduanChart1.resize();
							};
						}
					})
				})
