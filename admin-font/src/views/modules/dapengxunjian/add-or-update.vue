...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="巡检结果" prop="xunjianjieguo" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.xunjianjieguo" 
					class="editor"
					myQuillEditor="xunjianjieguo"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.xunjianjieguo" label="巡检结果" prop="xunjianjieguo"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.xunjianjieguo"></span>
			</el-form-item>
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='xunjianjieguo'){
							this.ruleForm.xunjianjieguo = obj[o];
							this.ro.xunjianjieguo = true;
							continue;
						}
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xunjianjieguo = this.ruleForm.xunjianjieguo.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `dapengxunjian/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.dapengxunjianCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
