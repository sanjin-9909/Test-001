	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="设备类型" prop="shebeileixing">
				<el-select v-model="ruleForm.shebeileixing" placeholder="请选择设备类型" :disabled="ro.shebeileixing"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="设备品牌" prop="shebeipinpai">
				<el-input v-model="ruleForm.shebeipinpai" 
					placeholder="设备品牌" clearable :readonly="ro.shebeipinpai"></el-input>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/shebeileixing/shebeileixing', {}).then(res => {
					if (res.data.code == 0) {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`shebeixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
