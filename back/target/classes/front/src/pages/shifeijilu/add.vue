...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="施肥数量" prop="shifeishuliang">
				<el-input v-model="ruleForm.shifeishuliang" 
					placeholder="施肥数量" clearable :readonly="ro.shifeishuliang"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="施肥时间" prop="shifeishijian">
				<el-input v-model="ruleForm.shifeishijian" 
					placeholder="施肥时间" clearable :readonly="ro.shifeishijian"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="施肥周期" prop="shifeizhouqi">
				<el-input v-model="ruleForm.shifeizhouqi" 
					placeholder="施肥周期" clearable :readonly="ro.shifeizhouqi"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="施肥方案" prop="shifeifangan">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="施肥方案"
				...
				<el-button type="primary" size="small" @click="AILongClick('shifeifangan', '施肥方案', '32')">AI生成</el-button>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`shifeijilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shifeijilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
