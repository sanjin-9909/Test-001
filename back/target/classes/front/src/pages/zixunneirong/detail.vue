	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zixunneirong?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">适用场景</div>
						<div class="text "  >{{detail.shiyongchangjing}}</div>
					</div>
					<div class="item">
						<div class="lable">难度等级</div>
						<div class="text "  >{{detail.nandudengji}}</div>
					</div>
					<div class="item">
						<div class="lable">核心点</div>
						<div class="text "  >{{detail.hexindian}}</div>
					</div>
					<div class="item">
						<div class="lable">步骤流程</div>
						<div class="text "  >{{detail.buzhouliucheng}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">适用人群</div>
						<div class="text "  >{{detail.shiyongrenqun}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zixunneirong','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zixunneirong','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('zixunneirong','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
						<el-button class="auditBtn" v-if="btnAuth('zixunneirong','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('zixunneirong','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="资讯详情" name="1">
					<div class="ql-snow ql-editor" v-html="detail.zixunxiangqing"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						if(this.detail.zixuntupian) {
							this.detailBanner = this.detail.zixuntupian.split(",w").length>1?[this.detail.zixuntupian]:this.detail.zixuntupian.split(',');
						}
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zixunneirong', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zixunneirongAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此资讯内容？') .then(_ => {
					this.$http.post('zixunneirong/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
