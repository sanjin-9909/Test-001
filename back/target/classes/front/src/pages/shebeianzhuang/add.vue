...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="运行状态" prop="yunxingzhuangtai">
				<el-select v-model="ruleForm.yunxingzhuangtai" placeholder="请选择运行状态" :disabled="ro.yunxingzhuangtai"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="安装时间" prop="anzhuangshijian">
				<el-date-picker
					:disabled="ro.anzhuangshijian"
			...
			<el-form-item class="add-item" label="安装详情" prop="anzhuangxiangqing">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`shebeianzhuang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('shebeianzhuang/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`shebeianzhuang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
