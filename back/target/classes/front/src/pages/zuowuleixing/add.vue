	...
	export default {
		data() {
			return {
				...
				rules: {
					zuowuleixing: [
						{ required: true, message: '作物类型不能为空', trigger: 'blur' },
					],
					zhouqi: [
					],
				},
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`zuowuleixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zuowuleixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
