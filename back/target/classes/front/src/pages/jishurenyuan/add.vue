...
<script>
	export default {
		data() {
			return {
				...
				rules: {
					...
					jishuyuanxingming: [
						{ required: true, message: '技术员姓名不能为空', trigger: 'blur' },
					],
					xingbie: [
					],
					touxiang: [
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`jishurenyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jishurenyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
			...
			zizhizhengshuUploadChange(fileUrls) {
				this.ruleForm.zizhizhengshu = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
