	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="生长阶段" prop="shengzhangjieduan">
				<el-select v-model="ruleForm.shengzhangjieduan" placeholder="请选择生长阶段" :disabled="ro.shengzhangjieduan"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="叶片数量" prop="yepianshuliang">
				<el-input v-model="ruleForm.yepianshuliang" 
					placeholder="叶片数量" clearable :readonly="ro.yepianshuliang"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="作物高度" prop="zuowugaodu">
				<el-input v-model="ruleForm.zuowugaodu" 
					placeholder="作物高度" clearable :readonly="ro.zuowugaodu"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`guoshushengzhang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`guoshushengzhang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
