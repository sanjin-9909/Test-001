<template>
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-input v-model="formSearch.zuowuleixing" placeholder="作物类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('guoshushengzhang','新增')" type="primary" @click="add('/index/guoshushengzhangAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('guoshushengzhang','果蔬生长数据')" type="success" @click="chartDialogShow1">
		...
		<el-dialog
			...
			width="800">
			<div id="shengzhangjieduanChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`guoshushengzhang/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/guoshushengzhangDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("guoshushengzhang/group/shengzhangjieduan",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chapterClick (row){
				...
				this.$router.push({path: '/index/chapterguoshushengzhang', query: params});
			},
		}
	}
</script>
