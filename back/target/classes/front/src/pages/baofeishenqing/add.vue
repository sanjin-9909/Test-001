	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="大棚名称" prop="dapengmingcheng">
				<el-select v-model="ruleForm.dapengmingcheng" placeholder="请选择大棚名称" :disabled="ro.dapengmingcheng"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="设备名称" prop="shebeimingcheng">
				<el-select  @change="shebeimingchengChange" v-model="ruleForm.shebeimingcheng" placeholder="请选择设备名称" :disabled="ro.shebeimingcheng" filterable>
					<el-option
			...
			<el-form-item class="add-item" label="报废原因" prop="baofeiyuanyin">
				<editor 
					v-model="ruleForm.baofeiyuanyin" 
					class="editor" 
					myQuillEditor="baofeiyuanyin"
					action="file/upload">
				</editor>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/guoshudapeng/dapengmingcheng', {}).then(res => {
					if (res.data.code == 0) {
						this.dapengmingchengOptions = res.data.data;
					}
				});
				this.$http.get('option/shebeixinxi/shebeimingcheng', {}).then(res => {
					if (res.data.code == 0) {
						this.shebeimingchengOptions = res.data.data;
					}
				});
			...
			shebeimingchengChange () {
				this.$http.get('follow/shebeixinxi/shebeimingcheng?columnValue=' + this.ruleForm.shebeimingcheng, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						if(res.data.data.shebeileixing){
							this.ruleForm.shebeileixing = res.data.data.shebeileixing
						}
					}
				});
			},
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`baofeishenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`baofeishenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
