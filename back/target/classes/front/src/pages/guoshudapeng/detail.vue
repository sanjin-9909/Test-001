	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/guoshudapeng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('guoshudapeng','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('guoshudapeng','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('guoshudapeng','申请')" 
							@click="onAcross('dapengshenqing','','','[1]','已申请')" type="warning">
							申请
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('guoshudapeng','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="设备设施" name="1">
					<div class="ql-snow ql-editor" v-html="detail.shebeisheshi"></div>
				</el-tab-pane>
			</el-tabs>
...
<script>
    ...
    export default {
      methods: {
        // 大棚申请方法
        applyDapeng() {
          this.$axios.post("/api/dapeng/apply", {
            dapengId: this.$route.query.id, // 大棚ID
            userId: this.$store.state.user.id, // 当前农户ID
            applyTime: new Date()
          }).then(res => {
            if (res.code === 200) {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`guoshudapeng`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/guoshudapeng', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/guoshudapengAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此果蔬大棚？') .then(_ => {
					this.$http.post('guoshudapeng/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterguoshudapeng', query: params});
			},
			// 获取uuid
			getUUID() {
