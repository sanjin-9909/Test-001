		...
		<el-form
			...
			<el-form-item class="add-item" label="大棚类型" prop="dapengleixing">
				<el-input v-model="ruleForm.dapengleixing" 
					placeholder="大棚类型" clearable :readonly="ro.dapengleixing"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="异常原因" prop="yichangyuanyin">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`shujuchakan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shujuchakan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
