	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/nonghu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('nonghu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('nonghu','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('nonghu','私聊')&&detail.id!=mid" @click="chatClick">在线咨询</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('nonghu','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.nonghuxingming;
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/nonghu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此农户？') .then(_ => {
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapternonghu', query: params});
			},
			// 获取uuid
			getUUID() {
