	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="协助时间" prop="xiezhushijian" >
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.xiezhushijian" 
						type="date"
						:readonly="ro.xiezhushijian"
						placeholder="协助时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.xiezhushijian" label="协助时间" prop="xiezhushijian" >
					<el-input v-model="ruleForm.xiezhushijian" placeholder="协助时间" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="协助详情" prop="xiezhuxiangqing" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.xiezhuxiangqing" label="协助详情" prop="xiezhuxiangqing"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.xiezhuxiangqing"></span>
			</el-form-item>
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xiezhuxiangqing = this.ruleForm.xiezhuxiangqing.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yujingxiezhu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yujingxiezhuCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
