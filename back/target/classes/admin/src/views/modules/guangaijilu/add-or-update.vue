	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="灌溉数量" prop="guangaishuliang" >
					<el-input v-model="ruleForm.guangaishuliang" placeholder="灌溉数量" clearable  :readonly="ro.guangaishuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="灌溉数量" prop="guangaishuliang" >
					<el-input v-model="ruleForm.guangaishuliang" placeholder="灌溉数量" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="灌溉时间" prop="guangaishijian" >
					<el-input v-model="ruleForm.guangaishijian" placeholder="灌溉时间" clearable  :readonly="ro.guangaishijian"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="灌溉时间" prop="guangaishijian" >
					<el-input v-model="ruleForm.guangaishijian" placeholder="灌溉时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="灌溉周期" prop="guangaizhouqi" >
					<el-input v-model="ruleForm.guangaizhouqi" placeholder="灌溉周期" clearable  :readonly="ro.guangaizhouqi"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="灌溉周期" prop="guangaizhouqi" >
					<el-input v-model="ruleForm.guangaizhouqi" placeholder="灌溉周期" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="灌溉方案" prop="guangaifangan" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="灌溉方案"
					v-model="ruleForm.guangaifangan" >
				</el-input>
				<el-button class="viewBtn" type="text" size="small" @click="AILongClick('guangaifangan', '灌溉方案', '32')">AI生成</el-button>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.guangaifangan" label="灌溉方案" prop="guangaifangan"  class="textBox">
				<span class="text">{{ruleForm.guangaifangan}}</span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='guangaishuliang'){
							this.ruleForm.guangaishuliang = obj[o];
							this.ro.guangaishuliang = true;
							continue;
						}
						if(o=='guangaishijian'){
						...
						if(o=='guangaizhouqi'){
							this.ruleForm.guangaizhouqi = obj[o];
							this.ro.guangaizhouqi = true;
							continue;
						}
						if(o=='guangaifangan'){
							this.ruleForm.guangaifangan = obj[o];
							this.ro.guangaifangan = true;
							continue;
						}
						if(o=='nonghuzhanghao'){
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `guangaijilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.guangaijiluCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			AILongClick(e, name, colLength) {
				...
				if(this.ruleForm.guangaishuliang) {
					text += ('灌溉数量 ' + this.ruleForm.guangaishuliang + '，')
				}
				if(this.ruleForm.guangaizhouqi) {
					text += ('灌溉周期 ' + this.ruleForm.guangaizhouqi + '，')
				}
