		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="维修进度" prop="weixiujindu" >
					<el-select :disabled="ro.weixiujindu" v-model="ruleForm.weixiujindu" placeholder="请选择维修进度"  filterable>
						<el-option
				...
				<el-form-item v-else class="input" label="维修进度" prop="weixiujindu" >
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="报修详情" prop="baoxiuxiangqing" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.baoxiuxiangqing" label="报修详情" prop="baoxiuxiangqing"  class="textBox">
	...
	export default {
		...
		methods: {
			...
			init(id,type ) {
				...
				this.weixiujinduOptions = "已维修,未维修".split(',')
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.baoxiuxiangqing = this.ruleForm.baoxiuxiangqing.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `guzhangbaoxiu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.guzhangbaoxiuCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
