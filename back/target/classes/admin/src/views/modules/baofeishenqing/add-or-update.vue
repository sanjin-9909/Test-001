		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'" label="设备名称" prop="shebeimingcheng" >
					<el-select :disabled="ro.shebeimingcheng" @change="shebeimingchengChange" v-model="ruleForm.shebeimingcheng" placeholder="请选择设备名称" filterable>
						<el-option
				...
				<el-form-item class="input" v-else-if="ruleForm.shebeimingcheng" label="设备名称" prop="shebeimingcheng" >
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="报废原因" prop="baofeiyuanyin" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.baofeiyuanyin" label="报废原因" prop="baofeiyuanyin"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.baofeiyuanyin"></span>
			</el-form-item>
		...
		methods: {
			...
			shebeimingchengChange () {
				this.$http({
					...
					if (data && data.code === 0) {
						if(data.data.shebeileixing){
							this.ruleForm.shebeileixing = data.data.shebeileixing
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.baofeiyuanyin = this.ruleForm.baofeiyuanyin.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
