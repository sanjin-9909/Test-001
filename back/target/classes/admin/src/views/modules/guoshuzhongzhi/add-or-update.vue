	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'" label="作物类型" prop="zuowuleixing" >
					<el-select :disabled="ro.zuowuleixing" @change="zuowuleixingChange" v-model="ruleForm.zuowuleixing" placeholder="请选择作物类型" filterable>
						<el-option
				...
				<el-form-item class="input" v-else-if="ruleForm.zuowuleixing" label="作物类型" prop="zuowuleixing" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="适宜温度" prop="shiyiwendu" >
					<el-input v-model.number="ruleForm.shiyiwendu" placeholder="适宜温度" clearable  :readonly="ro.shiyiwendu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="适宜温度" prop="shiyiwendu" >
				...
				<el-form-item class="input" v-if="type!='info'"  label="适宜湿度" prop="shiyishidu" >
					<el-input v-model.number="ruleForm.shiyishidu" placeholder="适宜湿度" clearable  :readonly="ro.shiyishidu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="适宜湿度" prop="shiyishidu" >
				...
				<el-form-item v-else class="input" label="种植时间" prop="zhongzhishijian" >
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="种植详情" prop="zhongzhixiangqing" >
				<editor 
			...
			<el-form-item v-else-if="ruleForm.zhongzhixiangqing" label="种植详情" prop="zhongzhixiangqing"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.zhongzhixiangqing"></span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			zuowuleixingChange () {
				this.$http({
					url: `follow/zuowuleixing/zuowuleixing?columnValue=`+ this.ruleForm.zuowuleixing,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.zhouqi){
							this.ruleForm.zhouqi = data.data.zhouqi
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.zhongzhixiangqing = this.ruleForm.zhongzhixiangqing.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `guoshuzhongzhi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.guoshuzhongzhiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
