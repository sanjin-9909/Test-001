...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					...
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select" label="作物类型" prop="zuowuleixing">
						...
						<el-select clearable v-model="searchForm.zuowuleixing" placeholder="请选择作物类型" >
							<el-option v-for="(item,index) in zuowuleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('guoshuzhongzhi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('guoshuzhongzhi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('guoshuzhongzhi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('guoshuzhongzhi','灌溉')" @click="guangaijiluCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								灌溉
							</el-button>
							<el-button class="btn8" v-if="isAuth('guoshuzhongzhi','施肥')" @click="shifeijiluCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								施肥
							</el-button>
							<el-button class="btn8" v-if="isAuth('guoshuzhongzhi','果蔬生长')" @click="guoshushengzhangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								果蔬生长
							</el-button>
							<el-button class="edit" v-if=" isAuth('guoshuzhongzhi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('guoshuzhongzhi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<guangaijilu-cross-add-or-update v-if="guangaijiluCrossAddOrUpdateFlag" :parent="this" ref="guangaijiluCrossaddOrUpdate"></guangaijilu-cross-add-or-update>
		<shifeijilu-cross-add-or-update v-if="shifeijiluCrossAddOrUpdateFlag" :parent="this" ref="shifeijiluCrossaddOrUpdate"></shifeijilu-cross-add-or-update>
		<guoshushengzhang-cross-add-or-update v-if="guoshushengzhangCrossAddOrUpdateFlag" :parent="this" ref="guoshushengzhangCrossaddOrUpdate"></guoshushengzhang-cross-add-or-update>



		<template v-if="statType">
			<div class="statShow2">
				<div class="echarts1" v-if="isAuth('guoshuzhongzhi','监测数据')">
					<div id="dapengmingchengChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('guoshuzhongzhi','热门作物')">
					<div id="zuowuleixingChart2" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	import guangaijiluCrossAddOrUpdate from "../guangaijilu/add-or-update";
	import shifeijiluCrossAddOrUpdate from "../shifeijilu/add-or-update";
	import guoshushengzhangCrossAddOrUpdate from "../guoshushengzhang/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				guangaijiluCrossAddOrUpdateFlag: false,
				shifeijiluCrossAddOrUpdateFlag: false,
				guoshushengzhangCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			guangaijiluCrossAddOrUpdate,
			shifeijiluCrossAddOrUpdate,
			guoshushengzhangCrossAddOrUpdate,
		},
		methods: {
			statChartClick() {
				if(this.isAuth('guoshuzhongzhi','监测数据')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('guoshuzhongzhi','热门作物')) {
			...
			guangaijiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.guangaijiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshuzhongzhi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.guangaijiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.guangaijiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			shifeijiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shifeijiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshuzhongzhi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shifeijiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shifeijiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			guoshushengzhangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.guoshushengzhangCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','guoshuzhongzhi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.guoshushengzhangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.guoshushengzhangCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "guoshuzhongzhi/valueMul/dapengmingcheng?yColumnNameMul=shiyiwendu,shiyishidu,guangzhao",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							legendObj.data = [
								'适宜温度'.split('/').length>1?'适宜温度'.split('/')[0]:'适宜温度',
								'适宜湿度'.split('/').length>1?'适宜湿度'.split('/')[0]:'适宜湿度',
							...
							let seriesObj = [
								{
									data: yAxis1,
									type: '适宜温度'.split('/').length>1?'适宜温度'.split('/')[1]:'line',
									name: '适宜温度'.split('/').length>1?'适宜温度'.split('/')[0]:'适宜温度',
								},
								{
									data: yAxis2,
									type: '适宜湿度'.split('/').length>1?'适宜湿度'.split('/')[1]:'line',
									name: '适宜湿度'.split('/').length>1?'适宜湿度'.split('/')[0]:'适宜湿度',
								},
								{
									data: yAxis3,
			...
			chartDialog2() {
				this.$nextTick(()=>{
					var zuowuleixingChart2 = echarts.init(document.getElementById("zuowuleixingChart2"),'macarons');
					let params = {}
					this.$http({
						url: "guoshuzhongzhi/group/zuowuleixing",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].zuowuleixing);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].zuowuleixing
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							zuowuleixingChart2.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								zuowuleixingChart2.resize();
							};
						}
					})
				})
			...
			init () {
				this.$http({
					url: `option/zuowuleixing/zuowuleixing`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.zuowuleixingOptions = data.data;
					} else {
