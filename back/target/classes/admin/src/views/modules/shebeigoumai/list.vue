...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('shebeigoumai','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shebeigoumai','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shebeigoumai','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shebeigoumai','安装')" @click="shebeianzhuangCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','已安装')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								安装
							</el-button>
							<el-button class="edit" v-if=" isAuth('shebeigoumai','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shebeigoumai','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('shebeigoumai','销售收益')">
					<div id="goumaijineChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	export default {
		...
		methods: {
			statChartClick() {
				if(this.isAuth('shebeigoumai','销售收益')) {
			...
			shebeianzhuangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shebeianzhuangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var goumaijineChart1 = echarts.init(document.getElementById("goumaijineChart1"),'macarons');
					let params = {}
					this.$http({
						url: "shebeigoumai/value/shebeimingcheng/goumaijine",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							goumaijineChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								goumaijineChart1.resize();
							};
						}
					})
				})
