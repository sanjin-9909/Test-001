			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"bold","height":"40px"}' class="item-label">技术员账号</label>
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('jishurenyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('jishurenyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('jishurenyuan','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true' prop="status" label="状态">
						<template slot-scope="scope">
							<el-switch
								...
								active-color="#10C17C"
								inactive-color="#ccc"
								@change="(e)=>jishurenyuanstatusChange(e,scope.row)">
							</el-switch>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jishurenyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('jishurenyuan','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="btn8" v-if="isAuth('jishurenyuan','私聊')" type="success" @click="chatClick(scope.row)">
								<span class="icon iconfont icon-zhangjie7" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								在线咨询							</el-button>



							<el-button class="del" v-if="isAuth('jishurenyuan','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			chatClick(row) {
				...
				if(row.jishuyuanzhanghao){
					this.nowname = row.jishuyuanzhanghao
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "jishurenyuan/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
			...
			jishurenyuanstatusChange(e,row){
				if(row.status==0){
					row.passwordwrongnum = 0
				}
				this.$http({
					url:'jishurenyuan/update',
					method:'post',
					data:row
				}).then(res=>{
					if(row.status==1){
