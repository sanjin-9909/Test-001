		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="采购价格" prop="caigoujiage" >
					<el-input-number v-model="ruleForm.caigoujiage" placeholder="采购价格" :disabled="ro.caigoujiage"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="采购价格" prop="caigoujiage" >
					<el-input v-model="ruleForm.caigoujiage" placeholder="采购价格" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="采购数量" prop="shuliang" >
					<el-input v-model.number="ruleForm.shuliang" placeholder="采购数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="采购数量" prop="shuliang" >
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="采购备注" prop="caigoubeizhu" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.caigoubeizhu" 
					class="editor"
					myQuillEditor="caigoubeizhu"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.caigoubeizhu" label="采购备注" prop="caigoubeizhu"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.caigoubeizhu"></span>
			</el-form-item>
	...
	export default {
		...
		computed: {
			...
			zongjine : {
				get: function () {
					let c = this.ruleForm
					let a = c.caigoujiage*c.shuliang
					this.ruleForm.zongjine = a?Number(Number(a).toFixed(2)):0
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.caigoubeizhu = this.ruleForm.caigoubeizhu.replace(reg,'../../../springboot584f4022/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							if(this.type=='cross'){
								...
								var objcross = this.$storage.getObj('crossObj');
								var table = this.$storage.get('crossTable');
								objcross.shuliang = Number((parseFloat(objcross.shuliang) + parseFloat(this.ruleForm.shuliang)).toFixed(2))
								await this.$http({
									url: `${table}/update`,
									method: "post",
									data: objcross
