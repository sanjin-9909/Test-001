	...
	mounted() {
				...
				menus.forEach(item=>{
					if(item.roleName == this.$storage.get('role')) {
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '农户管理') {
							if(
										this.isAuth('nonghu','农户活跃度')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '农户统计',
									tableName: 'nonghustat',
								})
							}
						}
					})
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '果蔬大棚管理') {
							if(
										this.isAuth('guoshudapeng','大棚数量')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '果蔬大棚统计',
									tableName: 'guoshudapengstat',
								})
							}
						}
					})
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '大棚申请管理') {
							if(
										this.isAuth('dapengshenqing','环境数据')
										||this.isAuth('dapengshenqing','活动参与')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '大棚申请统计',
									tableName: 'dapengshenqingstat',
								})
							}
						}
					})
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '果蔬种植管理') {
							if(
										this.isAuth('guoshuzhongzhi','监测数据')
										||this.isAuth('guoshuzhongzhi','热门作物')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '果蔬种植统计',
									tableName: 'guoshuzhongzhistat',
								})
							}
						}
					})
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '果蔬生长管理') {
							if(
										this.isAuth('guoshushengzhang','果蔬生长数据')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '果蔬生长统计',
									tableName: 'guoshushengzhangstat',
								})
							}
						}
					})
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '设备采购管理') {
							if(
										this.isAuth('shebeicaigou','设备采购费用')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '设备采购统计',
									tableName: 'shebeicaigoustat',
								})
							}
						}
					})
  					item.backMenu.forEach(item1=>{
						if(item1.menu == '设备购买管理') {
							if(
										this.isAuth('shebeigoumai','销售收益')
							) {
								arr.push({
									allButtons: ['查看'],
									buttons: ['查看'],
									menu: '设备购买统计',
									tableName: 'shebeigoumaistat',
								})
							}
						}
					})
		...
		this.$http({
			...
			if (data && data.code === 0) {
				...
				if(sessionTable == 'jishurenyuan') {
					this.avatar = data.data.touxiang
				}
				if(sessionTable == 'hezuoshe') {
